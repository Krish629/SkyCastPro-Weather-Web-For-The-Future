...
function Map({ children, styles, ...props }: MapProps) {
  ...
  // Sync camera when center changes
  useEffect(() => {
    if (mapRef.current && isLoaded && props.center) {
  ...
  }, [props.center, isLoaded]);

  // Sync zoom when zoom changes
  useEffect(() => {
    if (mapRef.current && isLoaded && props.zoom !== undefined) {
